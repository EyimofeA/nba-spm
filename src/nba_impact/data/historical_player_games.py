...
def build_historical_espn_player_games(
    ...
    *,
    ...
    """Emit only official/V3/ESPN-reconciled player-games and a full QA ledger."""
    if not seasons:
    ...
    groups = {key: value.copy() for key, value in espn.groupby(["season", "game_id"], sort=False)}
    ...
    for record in official.itertuples(index=False):
        ...
        source = groups.get((season, game_id))
        issues: list[str] = []
    ...
    output_rows = pd.concat(rows, ignore_index=True) if rows else pd.DataFrame()
    columns = [
    ...
    output_rows = output_rows.rename(columns={"team": "team_tricode", "minutes_played": "minutes"})
    ...
    source_paths = [espn_source, official_source, *v3_paths]
    source_files = [
