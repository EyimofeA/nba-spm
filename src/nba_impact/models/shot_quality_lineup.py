...
def _shooter_quality(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    direct["attempts"] = 1
    outputs = []
    for shot_class, piece in [
        ("rim", direct.loc[direct["shot_class"].eq("rim")]),
        ("non_rim", direct.loc[direct["shot_class"].eq("non_rim")]),
        ("all", direct),
    ]:
        grouped = piece.groupby("shooter_id", as_index=False).agg(
            shooter_attempts=("attempts", "sum"),
            shooter_expected_points=("expected_points", "sum"),
            shooter_actual_points=("actual_points", "sum"),
        )
        grouped["shot_class"] = shot_class
        grouped["shooter_expected_points_per_shot"] = (
            grouped["shooter_expected_points"] / grouped["shooter_attempts"]
        )
        league_rate = float(piece["expected_points"].sum() / piece["attempts"].sum())
        grouped["shooter_quality_above_league_per_100_shots"] = (
            grouped["shooter_expected_points_per_shot"] - league_rate
        ) * 100.0
    ...
    return pd.concat(outputs, ignore_index=True).rename(columns={"shooter_id": "player_id"})
