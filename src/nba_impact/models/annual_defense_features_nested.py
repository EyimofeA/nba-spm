"""Nested forward selection of predeclared annual defense feature blocks."""
...
def _select_variant(inner_metrics: pd.DataFrame) -> str:
    summary = (
        inner_metrics.groupby(["variant", "added_features"], as_index=False)
...
def run_annual_defense_features_nested(
    ...
    *,
    ...
    """Select a defense feature block using only seasons before each outer fold."""
    ...
    required = set().union(*map(set, variants.values()))
    if missing := sorted(required - set(features.columns)):
        raise ValueError(f"Candidate defensive features are missing: {missing}")
    ...
    for outer_season in contract["outer_test_seasons"]:
        fold_inner_rows = []
        for inner_season in (outer_season - 2, outer_season - 1):
        ...
        selected_variant = _select_variant(pd.DataFrame(fold_inner_rows))
        ...
        for label, source_variant in (
            ("nested_selected", selected_variant),
            ("baseline", "baseline"),
        ):
            feature_names = variants[source_variant]
            model = _fit_model(
                _ridge(alpha), outer_train, feature_names, "target_defense"
            )
            prediction = model.predict(outer_test.loc[:, feature_names])
            fold_predictions[f"prediction_{label}"] = prediction
            outer_rows.append(
                {
    ...
    selected_variants = outer_metrics.loc[
        outer_metrics["variant"].eq("nested_selected"), "source_variant"
    ]
    selected_counts = Counter(selected_variants.tolist())
    source_hashes = {
    ...
    run_id = f"annual_defense_features_nested_v1_{identity}"
    output = Path(artifact_root) / "models" / "annual_defense_features_nested" / run_id
    ...
    run = {
        "run_id": run_id,
        "model_family": "annual_defensive_spm_nested_feature_blocks",
        ...
        "metrics": {
            ...
            "mean_selected_minus_baseline_rmse": mean_rmse_delta,
            "mean_selected_minus_baseline_correlation": mean_correlation_delta,
            "selected_variant_counts": dict(sorted(selected_counts.items())),
        },
        "decision": {
            "promote_nested_feature_selection": passed,
            "basis": "Predeclared forward outer-fold gate; 2025 was not used.",
        },
        "artifact_path": str(output.resolve()),
        "caveats": [
            "The target is noisy one-season RAPM, not ground truth.",
            "Only the predeclared feature blocks were compared.",
        ],
    }
