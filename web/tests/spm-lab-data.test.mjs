...
test("SPM Lab defaults can render the public metric benchmark", () => {
  ...
  assert.equal(lab.comparison.team_win_summary.length, 9);
  assert.equal(lab.comparison.team_win_folds.length, 36);
  assert.equal(lab.comparison.team_win_summary[0].metric, "mamba");
  assert.ok(lab.comparison.pairwise_correlations.length >= 9 * 9 * 3);
