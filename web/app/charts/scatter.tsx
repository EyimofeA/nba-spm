...
/** Exact score-margin coefficients from the JE-style categorical RAPM. */
export function ScoreStateBubble({ rows }: { rows: ScoreStateDatum[] }) {
  const { bodyRef, tip, bind } = useChartTip<ScoreStateDatum>();
  if (!rows.length) return <div className="empty">No score-state estimates.</div>;
  const width = 880;
  const height = 470;
  const pad = { left: 66, right: 24, top: 24, bottom: 54 };
  const xBound = Math.max(...rows.map((row) => Math.abs(row.margin)), 1);
  const yBound = symmetricBound(
    rows.map((row) => row.effect_points_per_100_vs_tie),
    2,
  );
  const x = linear([-xBound, xBound], [pad.left, width - pad.right]);
  const y = linear([-yBound, yBound], [height - pad.bottom, pad.top]);
  const most = Math.max(...rows.map((row) => row.possessions), 1);
  const radius = (count: number) => 3 + Math.sqrt(count / most) * 8;
  const color = (state: ScoreStateDatum["state"]) =>
    state === "Trailing"
      ? SERIES[1]
      : state === "Leading"
        ? SERIES[0]
  ...
  return (
    <ChartBody bodyRef={bodyRef}>
      <svg
        ...
        role="img"
        aria-label="Score lead against estimated scoring effect"
      >
        ...
        {niceTicks(-yBound, yBound, 7).map((tick) => (
          <g key={`y-${tick}`}>
            <line
              className={tick === 0 ? "zero-line" : "grid-line"}
              x1={pad.left}
        ...
        {rows.map((row) => (
          <circle
            ...
            className="dot"
            cx={x(row.margin)}
            cy={y(row.effect_points_per_100_vs_tie)}
            r={radius(row.possessions)}
        ...
        <text
          ...
          textAnchor="middle"
        >
          Offense lead before possession
        </text>
        <text
          className="axis-title"
          transform="rotate(-90)"
          ...
          textAnchor="middle"
        >
          Effect on points per 100
        </text>
      </svg>
      {tip && (
        <Tooltip x={tip.x} y={tip.y} container={tip.width} width={190}>
          <div className="tt-head">
            {tip.datum.state} by {Math.abs(tip.datum.margin)}
          </div>
          <TipRow
            label="Effect / 100"
            value={fmtRating(tip.datum.effect_points_per_100_vs_tie)}
            color={color(tip.datum.state)}
          />
          <TipRow label="Possessions" value={fmtInt(tip.datum.possessions)} />
        </Tooltip>
      )}
    </ChartBody>
  );
